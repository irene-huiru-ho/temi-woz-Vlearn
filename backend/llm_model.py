# import os
# from dotenv import load_dotenv
# import google.generativeai as genai
# from openai import OpenAIError
# from pydantic import BaseModel

# load_dotenv()

# # gpt_client = OpenAI()
# load_dotenv()
# GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
# if not GEMINI_API_KEY:
#     raise RuntimeError("Missing GEMINI_API_KEY")
# genai.configure(api_key=GEMINI_API_KEY)

# GEMINI_CHAT_MODEL = "gemini-1.5-flash-002"
# # OPENAI_GPT_MODEL="gpt-4.1-mini-2025-04-14"
# # OPENAI_GPT_MODEL_LG="gpt-4.1-2025-04-14"
# FAMILY_INFO_STR = os.environ.get('FAMILY_INFO_STR')

# if not FAMILY_INFO_STR:
#     print('====================')
#     print('FAMILY_INFO_STR is not provided!')


# MAIN_PROMPT = '''
# **YOUR ROLE**
# You are a friendly, conversational social robot that can chat with the family and answer questions.


# The family info:
# {FAMILY_INFO}


# The latest USER input is: {COMMAND}.


# {IMAGE_PROMPT}


# In plain text, what is your response?
# '''.strip() + '\n\n\n'






# SYSTEM_PROMPT = '''
# You are a helpful agent controlling a domestic social robot,
# and your goal is to promote family-connection making.
# The transcriptions you are provided with may be from various people within the family.
# '''


# def generate_response(all_messages, img_path=None):
#     # msg_sys = {
#     #     'role': 'system',
#     #     'content': SYSTEM_PROMPT
#     # }
    
#     # if img_path is None:
#     #     prompt = MAIN_PROMPT.format(
#     #         FAMILY_INFO=FAMILY_INFO_STR,
#     #         COMMAND=all_messages[-1]['content'],
#     #         IMAGE_PROMPT=''
#     #     )
#     #     msg_last = {
#     #         'role': 'user',
#     #         'content': prompt
#     #     }
#     # else:
#     #     prompt = MAIN_PROMPT.format(
#     #         FAMILY_INFO=FAMILY_INFO_STR,
#     #         COMMAND=all_messages[-1]['content'],
#     #         IMAGE_PROMPT='You are also provided with the picture of the view in front of you.'
#     #     )
#     #     msg_last = {
#     #         'role': 'user',
#     #         'content': prompt
#     #     }
#     #     # TODO
#     #     msg_last = {
#     #         'role': 'user',
#     #         "content": [
#     #             {"type": "text", "text": prompt},
#     #             {
#     #                 "type":"file",
#     #                 "file":{
#     #                     "file_data":"data:application/pdf;base64,JVBERi0xLjUNJeLjz9MN...",
#     #                     "filename":"competion-sidebar.pdf"
#     #                 }
#     #             }
#     #         ],
#     #     }

#     # messages = [msg_sys] + all_messages[:-1] + [msg_last]
#     # res = ''
#     # try:
#     #     ans = gpt_client.chat.completions.create(
#     #         model=OPENAI_GPT_MODEL,
#     #         max_completion_tokens=256,
#     #         stop="\n\n\n",
#     #         messages=messages,
#     #         temperature=0.2,
#     #         top_p=1,
#     #         n=1,
#     #     )

#     #     res = ans.choices[0].message.content
#     #     token_output = ans.usage.completion_tokens
#     # except Exception as e:
#     #     print(f'[ERROR][generate_response]: {e}')
#     # return res


#     last_user = all_messages[-1]["content"]
#     if img_path:
#         image_prompt = "You are also provided with the picture of the view in front of you."
#     else:
#         image_prompt = ""

#     formatted = MAIN_PROMPT.format(
#         FAMILY_INFO=os.environ.get("FAMILY_INFO_STR", ""),
#         COMMAND=last_user,
#         IMAGE_PROMPT=image_prompt
#     )

#     chat_messages = []
#     chat_messages.append({"author": "system", "content": SYSTEM_PROMPT})
#     for msg in all_messages[:-1]:
#         chat_messages.append({
#             "author": msg["role"],
#             "content": msg["content"]
#         })
#     chat_messages.append({"author": "user", "content": formatted})
#     print("genai.chat is:", genai.chat)
#     try:
#         response = genai.chat.completions.create(
#             model=GEMINI_CHAT_MODEL,
#             messages=chat_messages,
#             temperature=0.2,
#             max_output_tokens=256,
#         )
#         return response.choices[0].message.content
#     except OpenAIError as e:
#         print(f"[ERROR] Gemini chat failed: {e}")
#         return "Sorry, I ran into an error."

import google.generativeai as genai
from PIL import Image
from openai import OpenAIError

GEMINI_CHAT_MODEL = "gemini-1.5-flash-002"
SYSTEM_PROMPT = """You are a helpful agent controlling a domestic social robot…, please respond in less than 30 words"""

def generate_response(history, img_path=None):
    msgs = [{"author": "system",    "content": SYSTEM_PROMPT}]
    msgs += [
        {"author": msg["role"], "content": msg["content"]}
        for msg in history
    ]

    if img_path:
        msgs.append({
            "author":  "user",
            "content": f"[Image provided at '{img_path}']"
        })

    try:
        response = genai.chat(
            model=GEMINI_CHAT_MODEL,
            messages=msgs,
            temperature=0.2,
            max_output_tokens=256,
        )
        return response.last.text

    except OpenAIError as e:
        print(f"[ERROR] Gemini chat failed: {e}")
        return "Sorry, I ran into an error."